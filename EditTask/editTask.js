...
class EditTask {
  ...
  addNote() {
    ...
    function removeAddNoteBtn() {
      document.querySelector(".add-note-btn").remove();
    }

    function showTextArea(noteTextArea) {
      document.querySelector(".task-detail__note").appendChild(noteTextArea);
    }
  }
  ...
  getTaskInfo() {
    const title = document.querySelector('input[name="task-detail__title"]').value
    const numPomodoros = document.querySelector('input[name="number-pomodoros"]').value;
    let note = '';
...
container.appendChild(EditTask.prototype.render());
